...
{#if show}
    <div class="ui-notify fixed z-1001 rounded shadow bg-white p2 min-w-240px dark:bg-dark-500 dark:shadow-main {customClass}"
        ...
        <div class="w-full flex items-center justify-between">
            ...
            {:else }
                <h1 class="flex-c">
                    {#if type}
                        <Icon icon={curType().icon}
                              on:click={handleClose}
                              colorCls="{curType().color} mr-2">
                        </Icon>
                    {/if}
                    {title}
                </h1>
