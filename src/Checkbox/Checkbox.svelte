...
<label class="inline-flex cursor-pointer justify-start text-tx dark:text-white  items-center gap-2 {customClass} { disabled ?  'ui-disabled-cursor' : ''}"
       {...attrs}>
    <input
            value={valueInner}
            disabled={disabled}
            ...
            hidden/>
    <div class="rounded border border-solid flex-c h-4 w-4 {valueInner ? 'bg-main' : ''} {classChecking} {disabled ? 'ui-disabled' : '' }">
        {#if valueInner}
