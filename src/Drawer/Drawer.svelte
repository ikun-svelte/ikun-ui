...
<Mask target={target}
    ...
    <div class="ui-drawer dark:bg-dark bg-white absolute z-1000 top-0 min-w-1/3 h-full p2 { placement === 'right' ? 'right-0' : 'left-0'} {customClass}"
         out:fly = {{duration: 250, x: placement === 'right' ? 200 : -200}}
         in:fly = {{duration: 250, x: placement === 'right' ? 200 : -200}}>
        <div class="w-24px h-24px">
            <Icon icon="{placement === 'right' ? 'i-carbon-chevron-right' : 'i-carbon-chevron-left'}"
        ...
        <div class="ui-drawer--content overflow-y-auto scroll-bar mt-2" style="height: calc(100% - 24px)">
            <slot></slot>
        </div>
    </div>
</Mask>
