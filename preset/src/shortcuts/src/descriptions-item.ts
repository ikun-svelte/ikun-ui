export const descriptionsItemShortcuts = {
	'k-descriptions-item--sm': 'text-12px',
	'k-descriptions-item--md': 'text-14px',
	'k-descriptions-item--lg': 'text-16px',

	'k-descriptions-item__horizontal': 'fi',
	'k-descriptions-item__vertical': 'flex flex-col',

	'k-descriptions-item--border':
		'b-t-1px b-t-solid b-t-ikun-bd-base b-l-1px b-l-solid b-l-ikun-bd-base',

	'k-descriptions-item--label': 'font-600 break-all flex-shrink-0 box-border',

	'k-descriptions-item--label__horizontal': 'h-100%',
	'k-descriptions-item--label__horizontal--sm': 'pr-8px',
	'k-descriptions-item--label__horizontal--md': 'pr-10px',
	'k-descriptions-item--label__horizontal--lg': 'pr-12px',

	'k-descriptions-item--label__vertical': 'b-r-none',
	'k-descriptions-item--label__vertical--sm': 'pb-8px',
	'k-descriptions-item--label__vertical--md': 'pb-10px',
	'k-descriptions-item--label__vertical--lg': 'pb-12px',

	'k-descriptions-item--label--sm--border':
		'!px-7px !py-4px b-r-1px b-r-solid b-r-ikun-bd-base bg-ikun-light-700 dark:bg-ikun-dark-100',
	'k-descriptions-item--label--md--border':
		'!px-11px !py-8px b-r-1px b-r-solid b-r-ikun-bd-base bg-ikun-light-700 dark:bg-ikun-dark-100',
	'k-descriptions-item--label--lg--border':
		'!px-15px !py-12px b-r-1px b-r-solid b-r-ikun-bd-base bg-ikun-light-700 dark:bg-ikun-dark-100',

	'k-descriptions-item--content': 'break-all flex-1 box-border',

	'k-descriptions-item--content__horizontal': '!b-t-none h-100%',

	'k-descriptions-item--content--sm--border': '!px-7px !py-4px b-t-1px b-t-solid b-t-ikun-bd-base',
	'k-descriptions-item--content--md--border': '!px-11px !py-8px b-t-1px b-t-solid b-t-ikun-bd-base',
	'k-descriptions-item--content--lg--border':
		'!px-15px !py-12px b-t-1px b-t-solid b-t-ikun-bd-base',

	// dark
	'k-descriptions-item__dark': 'dark:text-ikun-white'
};
