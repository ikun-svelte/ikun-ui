...
export default class {
	...
	updateParam(key: string, value: number | number[]) {
		if (this.param && key in this.param) {
			// if uniqueIds change, find out deleted id and remove from size map
			if (key === 'uniqueIds') {
				this.sizes.forEach((v, key) => {
					if (!(value as number[]).includes(key)) {
						this.sizes.delete(key);
					}
				});
			}
			this.param[key as keyof typeof this.param] = value as number & number[];
		}
	}
	...
	getEstimateSize() {
		return this.isFixedType() && this.fixedSizeValue !== undefined
			? this.fixedSizeValue
